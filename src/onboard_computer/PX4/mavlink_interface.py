...
class MAVLinkCommunicator:
    ...
    def connect_to_px4(self):
        try:
            ...
            print("mav ", self.mav)
            logging.info(
                f"Connected to PX4 on port {self.port} with baudrate {self.baudrate}"
            )
            ...
            try:
                ...
                if action in (0, 1, 2):  # 0: 悬停, 1: 前进, 2: 后退
                    ...
                    self.mav.mav.command_long_send(
                        ...
                        ),  # 0 表示减速到停止，1 表示加速到指定速度
                    )
                    logging.info(
                        "Action command (%s) sent to PX4",
                        "HOLD" if action == 0 else "MOVE",
                    )
                # 根据 direction 发送转向命令
                if direction in (0, 1):  # 0: 左转, 1: 右转
                    ...
                    logging.info(
                        "Direction command (%s) sent to PX4",
    ...
    def send_mission_message_to_px4(self, seq):
        ...
        try:
            ...
            logging.info(
                f"Sent command ack message for command {command} with result {result}"
            )
        except Exception as e:
        ...
        try:
            ...
            if self.check_heartbeat():  # 检查接收心跳的能力
                if self.test_send_capability():  # 检查发送能力
                    logging.info(
                        "Send capability test passed. Communication established."
                    )
                    return True
                else:
