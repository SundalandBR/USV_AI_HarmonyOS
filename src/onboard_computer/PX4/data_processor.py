...
class DataProcessor:
    ...
    def _parse_global_position(self, msg):
        ...
        if self.current_mission_item_seq != -1:
            if (
                abs(self.CURRENT_MISSION_ITEM["x"] - msg.lat) < 0.0001
                and abs(self.CURRENT_MISSION_ITEM["y"] - msg.lon) < 0.0001
            ):
    ...
    def _parse_sys_status(self, msg):
        ...
        self.SYS_STATUS["onboard_control_sensors_present"] = (
            msg.onboard_control_sensors_present
        )
        self.SYS_STATUS["onboard_control_sensors_enabled"] = (
            msg.onboard_control_sensors_enabled
        )
        self.SYS_STATUS["onboard_control_sensors_health"] = (
    ...
    def _parse_heartbeat(self, msg):
        ...
        # 判断无人艇是否处于启动状态
        if (
            self.HEARTBEAT["system_status"] == mavutil.mavlink.MAV_STATE_ACTIVE
            and self.HEARTBEAT["armed"]
        ):
            self.is_alive = True
        else:
            self.is_alive = False
