...
class model:
    def __init__(
        ...
        self.log_file = os.path.join(self.log_dir, "inference.log")
        self.error_log_file = os.path.join(self.log_dir, "inference_error.log")
        logging.basicConfig(
    ...
    def inference_seg_om(self, img_path=None, img=None):
        ...
        try:
            ...
            logging.error("model init error")
        # 预处理
        try:
            start = time.time()
            ...
            logging.error("img preprocess error")

        # 推理
        start = time.time()
        model = base.model(
            modelPath=self.model, deviceId=self.DEVICE_ID
        )  # noqa: F821 from mindx.sdk import base
        if model:
            ...
            if pred:
                ...
                if self.seg:
                    ...
                    output1 = torch.from_numpy(output1[0])
                    nm = 32
                else:
                    nm = 0
                # 后处理
                start = time.time()
                # output0 = np.array(output0).transpose((0,2,1))
                # pred = postprocess(output0)
                pred = nmx_v2(pred=torch.Tensor(output0), conf=0.25, iou=0.5, nm=nm)
    ...
    def mask(self):
        if self.pred.__len__() != 0:
            ...
            for i, mask in enumerate(masks):
                ...
                color = np.array(random_color(label))
                colored_mask = (
                    np.ones((img.shape[0], img.shape[1], 3)) * color
                ).astype(np.uint8)
                masked_colored_mask = cv2.bitwise_and(
                    colored_mask, colored_mask, mask=mask
                )
                mask_indices = mask == 1
                img[mask_indices] = (
