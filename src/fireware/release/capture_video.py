...
def get_folder_size(folder_path):
    ...
    for dir_path, _, filenames in os.walk(
        folder_path
    ):  # dir_path:正在遍历的目录路径  _:子目录列表  filenames:文件列表
        # 遍历文件
...
def capture_video(frame_queue, enable):
    ...
    try:
        ...
        while enable.is_set():
            ...
            # 创建视频对象
            out = cv2.VideoWriter(
                output_video, fourcc, fps, (video_width, video_height), isColor=True
            )
            if not out.isOpened():
                print("无法打开输出视频文件")
                cap.release()
            ...
            while enable.is_set():
                ...
                # 控制推理模块读取的帧率
                if (
                    current_time - image_start_time
                ) / cv2.getTickFrequency() >= frame_interval:
                ...
                ) / cv2.getTickFrequency() >= save_interval:
        ...
        while True:
            ...
            while True:
                ...
                _, img_encoded = cv2.imencode(
                    ".jpg", img1, [int(cv2.IMWRITE_JPEG_QUALITY), quality]
                )
